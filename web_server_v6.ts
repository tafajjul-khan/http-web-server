...
function readerFromFile(fp: fs.FileHandle, size: number): BodyReader {
  let remain = size;
  const kBufSize = 1024 * 4; 
  const buf = Buffer.alloc(kBufSize);
  ...
  return {
    ...
    read: async (): Promise<Buffer> => {
      ...
      const n = await fp.read(buf, 0, Math.min(kBufSize, remain));
      remain -= n.bytesRead;

      return Buffer.from(buf.subarray(0, n.bytesRead));
    },
  };
}
...
async function serveStaticFile(uri: string): Promise<HTTPRes> {
  const safePath = path.join(process.cwd(), "public", uri === "/" ? "index.html" : uri);
