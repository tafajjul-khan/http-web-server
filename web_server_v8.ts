...
import { pipeline } from "stream/promises"; // Sahi import
...
type BodyReader = {
  length: number;
  read: () => Promise<Buffer>;
  close?: () => Promise<void>; // Optional close for cleanup
};
...
function fieldGetList(headers: Buffer[], key: string): string[] {
  const field = fieldGet(headers, key);
  if (!field) {
    return [];
  }
  // Buffer ko string banayein, comma se split karein, 
  // aur extra spaces hata kar lowercase kar dein.
  return field
    .toString("latin1")
    .split(",")
...
function enableCompression(req: HTTPReq, res: HTTPRes): void {
  ...
  if (!codecs.some(c => c.includes("gzip"))) return;
...
function gzipFilter(reader: BodyReader): BodyReader {
  ...
  const input = body2stream(reader);
  ...
  (async () => {
    try {
      ...
      gz.destroy(err instanceof Error ? err : new Error("Gzip error"));
    }
  })();
  ...
  return {
    length: -1, // Unknown after compression
...
server.listen({ host: "127.0.0.4", port: 1234 }, () => {
  console.log("Server running at http://127.0.0.4:1234");
});
